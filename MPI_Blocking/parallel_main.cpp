...
int main(int argc, char *argv[]){
  ...
  for(size_t j = 0; j<measurements; ++j){    
    ...
    constexpr int test_case_id = 1;  // Water drops in a box
    ...
    MPI_Bcast(&my_ny, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&my_nx, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    std::vector<double> local_zdx_(local_size); //x derivative of topography
    std::vector<double> local_zdy_(local_size); //y derivative of topography
    ...
    MPI_Scatterv(global_zdx.data(), sendcounts.data(), displacements.data(), MPI_DOUBLE, local_zdx_.data(), local_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Scatterv(global_zdy.data(), sendcounts.data(), displacements.data(), MPI_DOUBLE, local_zdy_.data(), local_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    parallel_solver.set_sendcounts(sendcounts);
    parallel_solver.set_displacements(displacements);
  ...
  if(rank == 0 and measurements > 1){
    ...
    #endif
    std::cout << "Mean times for parallel SWE on "<<threads<<" thread and "<<size<<" cores = " << mean(times) << " [s]\n";
  }
