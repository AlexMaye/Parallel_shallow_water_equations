...
void
SWESolver::solve_gpu(const double Tend, const bool full_log, const std::size_t output_n, const std::string &fname_prefix){
  ...
  GpuSWESolver gpuSolver(nx_, ny_, size_x_, size_y_, Tend, reflective_);
  gpuSolver.upload_initial(h0_, hu0_, hv0_, zdx_, zdy_);

  //std::cout << "Solving SWE on GPU..." << std::endl;
  double T = 0.;
  double h_dt;
  std::size_t break_point = 0;

  
  std::vector<double> h(nx_*ny_);
  ...
  while(T<Tend){
    ...
    T += h_dt;
    if (output_n>0){
      printf("Computing T: %2.4f hr  (dt = %.2e s) -- %3.3f%%", T, h_dt * 3600, 100 * T / Tend);
      std::cout << (full_log ? "\n" : "\r") << std::endl<< std::flush;
    }

    if (output_n > 0 && nt % output_n == 0)
    {
      //std::cout << "Downloading h..."<<std::endl;
      gpuSolver.download(h);
  ...
  std::cout<<"Number of steps for "<<nx_<<"x"<<ny_<<" problem is "<<break_point<<std::endl;
  std::cout<<"Mean time per iteration = "<<mean(timings)<<std::endl;
}
...
SWESolver::solve(const double Tend, const bool full_log, const std::size_t output_n, const std::string &fname_prefix)
{
  ...
  while (T < Tend)
  {
    ...
    std::cout << (full_log ? "\n" : "\r") << std::endl<< std::flush;
    //std::cout << "print dt ="<<print_dt<<", T ="<<T1<<", dt ="<<dt<<std::endl;
